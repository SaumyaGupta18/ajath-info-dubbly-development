...
class MainVC: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        self.sideMenuViewController = storyboard.instantiateViewController(withIdentifier: "SideMenuID") as? SideMenuVC
...
extension MainVC: SideMenuViewControllerDelegate {
...
extension UIViewController {
    ...
    func revealViewController() -> MainVC? {
        ...
        if vc != nil && vc is MainVC {
            return vc! as? MainVC
        }
        while (!(vc is MainVC) && vc?.parent != nil) {
            vc = vc?.parent
        }
        if vc is MainVC {
            return vc as? MainVC
            ...
            if self.draggingIsEnabled {
                if self.revealSideMenuOnTop {
                    ...
                    if let recogView = sender.view?.subviews[1] {
                       // Show/Hide shadow background view while dragging
                      //  let percentage = (recogView.frame.origin.x * 150 / self.sideMenuRevealWidth) / textself.sideMenuRevealWidth
